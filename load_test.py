"""Load test for the embeddings worker -- observe Flash autoscaling.

Adapted from flash-examples/04_scaling_performance/01_autoscaling/load_test.py
(only DEFAULT_ENDPOINT and DEFAULT_PAYLOAD changed to hit /embeddings_worker).
...
Usage (run AFTER `flash dev` is up in another terminal):
    uv run python load_test.py
    uv run python load_test.py --concurrency 10 --requests 50
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load test for Flash autoscaling")
    parser.add_argument("--url", default=DEFAULT_URL, help=f"Base URL (default: {DEFAULT_URL})")
    parser.add_argument(
        "--endpoint", default=DEFAULT_ENDPOINT, help=f"Endpoint path (default: {DEFAULT_ENDPOINT})"
    )
    parser.add_argument("--concurrency", type=int, default=10, help="Max concurrent requests")
    parser.add_argument("--requests", type=int, default=20, help="Requests per burst")
    parser.add_argument("--pause", type=int, default=10, help="Pause between bursts (seconds)")
